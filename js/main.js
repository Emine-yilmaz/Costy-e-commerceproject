...
// Urunleri ekrana renderlayacak fonksiyon
...
function addToCart(event) {
  ...
  // Dizinin icerisinde tikladigimiz idli elemani bulmak icin find methodunu kullandik
  const product = products.find((product) => product.id === productID);

  if (product) {
    // Sepette tikladigimiz urun onceden var mi yok mu kontrol ederiz
    const exitingItem = cart.find((item) => item.id === productID);
    // Sepette tikladigimiz urun varsa bu urunun miktarini bir arttiririz
    if (exitingItem) {
      exitingItem.quantity++;
      console.log(cart);
      
    } else {
      // Tikladigimiz urun sepette yoksa yeni bir urun seklinde ekleriz
      const cartItem = {
        id: product.id,
        title: product.title,
        price: product.price,
...
function removeFromCart(event){
    const productID = parseInt(event.target.dataset.id);
    console.log(cart);
    // cart dizisinden silmek istedigimkiz urunu idsine gore cart dizisinden sildik
    cart = cart.filter((item) => item.id !== productID);
    // localStorage i guncelledik
    saveToLocalStorage();
    // sayfayi guncelledik
    renderCartItems();
...
// Localstoraga veri eklemek icin kullandik
...
function calculateCartTotal() {
    // reduce iki deger ister, bunlardan birincisi icerisinde yapacagimiz islem, ikincisi ise baslangic degeri
...
// Sayfa cart.html sayfasindaysa renderCartItems fonksiyonu calismasi gerekiyor
if (window.location.pathname.includes("cart.html")) {
  renderCartItems();
  calculateCartTotal();
} else {
    // Sayfa index.html sayfasindaysa renderProducts fonksiyonu calisacak
  renderProducts();
}
